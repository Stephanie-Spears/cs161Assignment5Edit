...
int RectangleValues(int width, int height);
...
int main()
{
	...
	cout << "BEGIN RECTANGLE'\n'";


	
	RectangleValues(width);
	

	RectangleValues(height);


	cout << "Your rectangle is " << width << " by " << height;
...
int RectangleValues(string Dimension, int width, int height)
{
	...
	return width, height; //should i be using return value? or should that be above the "for" statement, and "for" should be it's own print function?
}
